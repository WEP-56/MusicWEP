...
/// Wraps [child] in the custom window chrome on Windows desktop.
/// On other platforms returns [child] unchanged.
Widget buildDesktopWindowFrame({
  required BuildContext context,
  required Widget child,
}) {
  if (!_isWindowsDesktop) return child;
...
class _DesktopWindowFrameState extends ConsumerState<_DesktopWindowFrame>
  ...
  @override
  void onWindowClose() => unawaited(_applyCloseBehavior(_closeBehavior));
  ...
  @override
  void onWindowResized() => _schedulePersistWindowSize();

  @override
  void onTrayIconMouseDown() => unawaited(_showMainWindow());

  @override
  void onTrayIconRightMouseDown() => unawaited(_showTrayContextMenu());

  @override
  void onTrayIconRightMouseUp() => unawaited(_showTrayContextMenu());
  ...
  Future<void> _exitApplication() async {
    ...
    if (_trayReady) unawaited(trayManager.destroy());
    exit(0);
  }
}
